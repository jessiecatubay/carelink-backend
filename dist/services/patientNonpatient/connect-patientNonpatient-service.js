...
export async function ConnectPatientNonpatientService(nonPatientId, connectionCode) {
    ...
    try {
        ...
        const isConnected = await patientNonpatientRepository.findConnection(patientId.userId, nonPatientId);
        if (isConnected) {
            return {
                code: 500,
                status: "error",
